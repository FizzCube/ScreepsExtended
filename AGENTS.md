...
- **Store & Cache**: Data from hooks are stored by `src/radar-store.js`, terrain is cached in `src/terrain-cache.js`, and username mappings are managed by `src/user-cache.js`.
...
- **Testing**: Testing is manual (no test harness). Key techniques:
	...
	- Check localStorage for cache keys: `SMO_TERRAIN_CACHE_V1`, `SMO_USER_CACHE_V1`.
...
- **Renderer Modules**: Each entity type is implemented as a self-contained renderer in `src/renderers/`. Each renderer exports functionality by assigning a named object on `window`, e.g., `window.ScreepsTerrainRenderer`, `window.ScreepsCreepRenderer` and so on.
...
- `src/renderer-main.js` ? orchestrates the rendering pipeline and exposes `SMO.render()`.
- `src/overlay.js` ? creates and aligns canvases on the game's stage element.
- `src/config/constants.js` ? reserved keys, TYPE_STYLES, deposit types, and other config constants.
